...
val Explanation2 = state(Interaction) {
    onEntry {
        furhat.ask("A percentage is nothing more than a ratio" +
                "expressed as a fraction of 100. So we could also write 100% as 100/100, 50% as 50/100, 1% as 1/100 et cetera." +
                " Do you understand this so far?")
    }

    onResponse<Yes> { goto(Explanation2Example) }
    onResponse<Repeat> {
        furhat.say("Okay, I will repeat my explanation again.")
        reentry() }
    onResponse<No> {
...
val Explanation1Example: State = state(Interaction) {
    onEntry {
        furhat.say {"Let's try to compute 20% of 500. We start by calculating 1% of 500" + delay(250)}
        furhat.say {"In our example we need to divide 500 by 100 to get 1%. " +
                "This will give us 5." + delay(250)}
        furhat.say("Remember that this is only 1%, and we would like to know 20% of 500"
